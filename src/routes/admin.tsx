...
import { 
  LayoutDashboard, ShoppingBag, CheckSquare, Package, 
  Tags, LayoutTemplate, Users, Archive, Star, 
  Ticket, BarChart3, Edit3, Image as ImageIcon, 
  Settings, User, Menu, X, Search, Bell, LogOut, Moon, Sun
} from "lucide-react";
...
function AdminLayout() {
  ...
  if (isAuthLoading || isProfileLoading || profile?.role !== 'admin') {
    return <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900 flex items-center justify-center text-sm tracking-widest uppercase">Verifying Clearance...</div>;
  }
  ...
  return (
    <div className={`min-h-screen font-sans ${isDarkMode ? 'dark bg-neutral-950 text-neutral-100' : 'bg-neutral-50 text-neutral-900'}`}>
      ...
      `}>
        <div className="h-16 flex items-center justify-between px-6 border-b border-neutral-200 dark:border-neutral-800 sticky top-0 bg-white dark:bg-neutral-900 z-10">
          <Link to="/admin" className="font-serif text-xl tracking-widest font-bold">LENORAA</Link>
        ...
        <div className="p-4 space-y-8 pb-20">
          {menuGroups.map((group, idx) => (
            <div key={idx}>
              <div className="px-3 text-[10px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-500 mb-3">{group.title}</div>
      ...
      <div className="lg:pl-64 flex flex-col min-h-screen">
        ...
        <header className="h-16 bg-white dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            <button className="lg:hidden text-neutral-500 hover:text-neutral-900 dark:hover:text-white" onClick={() => setSidebarOpen(true)}>
              <Menu size={20} />
            </button>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button className="p-2 text-neutral-500 hover:text-neutral-900 dark:hover:text-white rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors relative">
              <Bell size={20} />
