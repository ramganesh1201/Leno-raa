...
function AdminOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order: any) => {
      const matchesSearch = 
        ...
        order.shipping_addresses?.[0]?.full_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleUpdateStatus = async (orderId: string, currentStatus: string, currentNotes: string, currentCourier: string, currentTracking: string) => {
    const newStatus = prompt("Enter new order status (Preparing, Packed, Shipped, Delivered, Cancelled):", currentStatus);
    if (!newStatus) return;
    ...
    await updateStatus.mutateAsync({ orderId, status: newStatus, notes: newNotes, courier: newCourier, tracking: newTracking });
    setSelectedOrder(null);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--muted-foreground)]" size={16} />
              <input 
                type="text" 
            ...
            <div className="relative">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--muted-foreground)]" size={16} />
              <select
              ...
              <tbody>
                {isError ? (
                  ...
                  filteredOrders.map((order: any) => (
                    <tr key={order.id} className="border-b border-[color:var(--border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer" onClick={() => setSelectedOrder(order)}>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-right">
                        <button className="text-[color:var(--gold)] hover:underline text-xs tracking-widest uppercase">View</button>
                      </td>
                    </tr>
                  ))
                )}
            ...
            >
              ...
              <div className="p-8 md:p-10">
                <div className="mb-8 border-b border-neutral-200 dark:border-neutral-800 pb-6">
                  <div className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Order Details</div>
                  ...
                  <div className="mt-4 flex gap-3">
                      ...
                      <span className={`inline-flex px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-medium border ${
                          selectedOrder.payment_status === 'Verified' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 
                          selectedOrder.payment_status === 'Rejected' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
                          selectedOrder.payment_status === 'Awaiting Verification' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                          'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'
                        }`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-8">
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-4 border-b border-neutral-200 dark:border-neutral-800 pb-2">Customer & Shipping</h3>
                      <div className="text-sm space-y-2 text-neutral-700 dark:text-neutral-300">
                          <p><span className="opacity-60 w-20 inline-block">Name:</span> <span className="font-medium">{selectedOrder.shipping_addresses?.[0]?.full_name}</span></p>
                          <p><span className="opacity-60 w-20 inline-block">Email:</span> {selectedOrder.profiles?.email}</p>
                          <p><span className="opacity-60 w-20 inline-block">Phone:</span> {selectedOrder.shipping_addresses?.[0]?.phone}</p>
                          <p><span className="opacity-60 w-20 inline-block align-top">Address:</span> <span className="inline-block w-[calc(100%-5rem)]">{selectedOrder.shipping_addresses?.[0]?.address}, {selectedOrder.shipping_addresses?.[0]?.city}, {selectedOrder.shipping_addresses?.[0]?.state} {selectedOrder.shipping_addresses?.[0]?.pincode}</span></p>
                        </div>
                      </div>

                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-4 border-b border-neutral-200 dark:border-neutral-800 pb-2">Order Items</h3>
                      <div className="space-y-3">
                        {selectedOrder.order_items?.map((item: any) => (
                          <div key={item.id} className="flex justify-between items-center text-sm bg-neutral-50 dark:bg-neutral-800/50 p-3 rounded-lg border border-neutral-200 dark:border-neutral-800">
                            <div>
                              <div className="font-medium text-neutral-900 dark:text-white">{item.product?.name || "Custom Soap"}</div>
                              <div className="text-xs text-neutral-500">Qty: {item.quantity} x ₹{item.price}</div>
                              </div>
                              <div className="font-medium tracking-wide">₹{item.quantity * item.price}</div>
                            </div>
                          ))}
                        </div>
                      <div className="mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-800 text-sm space-y-2">
                        ...
                        <div className="flex justify-between text-neutral-500">
                          <span>Shipping</span><span>₹{selectedOrder.shipping_cost}</span>
                        </div>
                  ...
                  <div className="space-y-8">
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-4 border-b border-neutral-200 dark:border-neutral-800 pb-2">Payment Verification</h3>
                      {selectedOrder.payment_proofs?.[0] ? (
                        <div className="space-y-4">
                          ...
                          <div className="text-sm bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-lg border border-neutral-200 dark:border-neutral-800">
                            <p><span className="opacity-60 text-neutral-500">UTR Number:</span> <span className="font-medium tracking-wide">{selectedOrder.payment_proofs[0].utr_number}</span></p>
                            <p className="mt-1"><span className="opacity-60 text-neutral-500">Uploaded:</span> {new Date(selectedOrder.payment_proofs[0].uploaded_at).toLocaleString()}</p>
                          </div>
                            
                            {selectedOrder.payment_status === 'Awaiting Verification' && (
                              <div className="flex gap-4 pt-2">
                                <button 
                                  onClick={() => handleVerify(selectedOrder.id, selectedOrder.payment_proofs[0].id, true)}
                                ...
                                <button 
                                  onClick={() => handleVerify(selectedOrder.id, selectedOrder.payment_proofs[0].id, false)}
                                  disabled={verifyPayment.isPending}
                      ...
                      <div>
                        <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-4 border-b border-neutral-200 dark:border-neutral-800 pb-2">Admin Actions</h3>
                        <div className="space-y-4">
                          <div className="text-sm bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-lg space-y-2 border border-neutral-200 dark:border-neutral-800">
                            <p><span className="opacity-60 text-neutral-500 w-16 inline-block">Courier:</span> {selectedOrder.courier_name || "N/A"}</p>
                            <p><span className="opacity-60 text-neutral-500 w-16 inline-block">Tracking:</span> {selectedOrder.tracking_number || "N/A"}</p>
                            <p><span className="opacity-60 text-neutral-500 w-16 inline-block">Notes:</span> {selectedOrder.internal_notes || "None"}</p>
                          </div>
                          <button 
                            onClick={() => handleUpdateStatus(selectedOrder.id, selectedOrder.order_status, selectedOrder.internal_notes, selectedOrder.courier_name, selectedOrder.tracking_number)}
...
function PaymentProofImage({ url }: { url: string }) {
  ...
  useEffect(() => {
    async function loadUrl() {
      try {
        ...
        if (url.includes('?token=') || (url.startsWith('http') && !url.includes('/storage/v1/object/public/'))) {
          setSignedUrl(url);
          return;
        }
  ...
  return (
    <a href={signedUrl} target="_blank" rel="noreferrer" className="block relative group border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden hover:border-blue-500 transition-colors">
      <img src={signedUrl} alt="Proof" className="w-full h-auto max-h-48 object-cover bg-neutral-100 dark:bg-neutral-800" onError={() => setError(true)} />
