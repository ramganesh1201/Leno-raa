...
function AdminCouponsPage() {
  ...
  const filteredCoupons = (coupons || []).filter((c: any) => c.code.toLowerCase().includes(searchQuery.toLowerCase()));
            ...
            <tbody>
              {isLoading ? (
                ...
                filteredCoupons.map((coupon: any) => (
                  <tr key={coupon.id} className={`border-b border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors ${!coupon.is_active ? 'opacity-50 grayscale' : ''}`}>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="text-sm space-y-1">
                        <div className="text-neutral-500">Min: <span className="font-medium text-neutral-900 dark:text-white">₹{coupon.min_order_amount || 0}</span></div>
                        {coupon.max_discount_amount && (
                          <div className="text-neutral-500">Max Disc: <span className="font-medium text-neutral-900 dark:text-white">₹{coupon.max_discount_amount}</span></div>
                        )}
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm font-medium">
                        {coupon.times_used || 0} {coupon.usage_limit ? `/ ${coupon.usage_limit}` : 'uses'}
                      </div>
                      {coupon.usage_limit && (coupon.times_used || 0) >= coupon.usage_limit && (
                        <div className="text-[10px] text-rose-500 font-bold uppercase tracking-widest mt-1">Limit Reached</div>
                      )}
                    </td>
                    <td className="p-4">
                      ...
                      {coupon.expires_at && new Date(coupon.expires_at) < new Date() && (
                        <div className="text-[10px] text-rose-500 font-bold uppercase tracking-widest mt-1">Expired</div>
                      )}
                    </td>
                    <td className="p-4">
                      <button 
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openEditModal(coupon)} className="p-1.5 text-neutral-400 hover:text-blue-500 transition-colors rounded">
                          <Edit3 size={16} />
                        </button>
                        <button onClick={() => handleDelete(coupon.id)} className="p-1.5 text-neutral-400 hover:text-rose-500 transition-colors rounded">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input type="text" value={code} onChange={e => setCode(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 font-mono uppercase outline-none" placeholder="e.g. SUMMER20" />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">Type</label>
                    <select value={type} onChange={e => setType(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 outline-none">
                  ...
                  <div>
                    ...
                    <input type="number" value={value} onChange={e => setValue(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 outline-none" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={minOrder} onChange={e => setMinOrder(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 outline-none" placeholder="Optional" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Max Discount (₹)</label>
                    <input type="number" value={maxDiscount} onChange={e => setMaxDiscount(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 outline-none" placeholder="Optional" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={usageLimit} onChange={e => setUsageLimit(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 outline-none" placeholder="Optional" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Expiry Date</label>
                    <input type="date" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 outline-none text-neutral-500" />
                  </div>
                </div>
              </div>
