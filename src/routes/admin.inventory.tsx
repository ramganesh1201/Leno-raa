    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-neutral-500 mt-1">Monitor stock levels, reserved units, and availability.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-neutral-500 font-medium uppercase tracking-wider">In Stock</div>
          </div>
        </div>
        <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-neutral-500 font-medium uppercase tracking-wider">Low Stock (&le;10)</div>
          </div>
        </div>
        <div className="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-neutral-500 font-medium uppercase tracking-wider">Out of Stock</div>
          </div>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-neutral-500">
                    <div className="animate-pulse space-y-4">
                      {[1,2,3,4].map(i => <div key={i} className="h-10 bg-neutral-100 dark:bg-neutral-800 rounded w-full"></div>)}
                    </div>
                  </td>
                </tr>
                    ...
                    <tr key={product.id} className="border-b border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-medium text-sm text-neutral-900 dark:text-white">{product.name}</div>
                            {stockStatus === 'out' && <span className="text-[10px] uppercase font-bold text-rose-500">Out of Stock</span>}
                            {stockStatus === 'low' && <span className="text-[10px] uppercase font-bold text-amber-500">Low Stock</span>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4 text-center">
                        <div className="font-mono text-sm text-neutral-400">{product.reserved_stock}</div>
                      </td>
                      <td className="p-4 text-center">
                        <div className={`font-mono font-bold text-sm ${
                          stockStatus === 'out' ? 'text-rose-600' : 
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <input 
                            ...
                            onChange={(e) => setLocalStock(prev => ({ ...prev, [product.id]: parseInt(e.target.value) || 0 }))}
                          />
                          <button 
