...
function OrderDetailsPage() {
  ...
  if (!order) {
    return (
      <div className="mt-12 text-center p-12 surface-glass rounded-[24px] border border-[color:var(--border)]">
        ...
        <Link to="/account/orders" className="btn-lux inline-flex">Back to Orders</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link to="/account/orders" className="flex items-center gap-2 text-sm uppercase tracking-widest text-[color:var(--muted-foreground)] hover:text-[color:var(--gold)] transition-colors mb-6">
          <ArrowLeft size={16} /> Back to Orders
        </Link>
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
          <div>
            <div className="text-eyebrow text-[color:var(--muted-foreground)] mb-2">Order Details</div>
            <h1 className="text-display text-3xl">{order.order_number}</h1>
          </div>
          <div className="flex gap-3">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs uppercase tracking-widest font-medium border ${
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="surface-glass p-8 rounded-[24px] border border-[color:var(--border)] shadow-sm">
            ...
            <div className="space-y-4">
              {order.order_items?.map((item: any) => (
                <div key={item.id} className="flex items-center gap-4 bg-black/5 dark:bg-white/5 p-4 rounded-xl border border-[color:var(--border)]">
          ...
          {(order.payment_status === 'Pending' || order.payment_status === 'Rejected') && (
            <div className="surface-glass p-8 rounded-[24px] border border-[color:var(--border)] shadow-sm flex justify-between items-center bg-[color:var(--gold)]/5 border-[color:var(--gold)]/30">
              <div>
                <h3 className="text-display text-xl mb-2 text-[color:var(--gold)]">Payment Required</h3>
                <p className="text-sm text-[color:var(--muted-foreground)]">Please complete your payment to process this order.</p>
              </div>
              <Link to="/payment/$orderId" params={{ orderId: order.id }} className="btn-lux shrink-0">
                Complete Payment
              </Link>
            </div>
          )}
          ...
          {proof && (
            <div className="surface-glass p-8 rounded-[24px] border border-[color:var(--border)] shadow-sm">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start">
                <a href={proof.screenshot_url} target="_blank" rel="noreferrer" className="block shrink-0 border border-[color:var(--border)] rounded-xl overflow-hidden hover:border-[color:var(--gold)] transition-colors relative group">
                  <img src={proof.screenshot_url} alt="Payment Proof" className="w-48 h-auto object-cover bg-black/5" />
                ...
                <div className="space-y-4 flex-1">
                  <div>
                    <div className="text-xs uppercase tracking-widest text-[color:var(--muted-foreground)] mb-1">UTR Number</div>
                    <div className="text-sm font-medium tracking-wide">{proof.utr_number}</div>
                  </div>
                  <div>
                    <div className="text-xs uppercase tracking-widest text-[color:var(--muted-foreground)] mb-1">Uploaded On</div>
                  ...
                  {proof.rejection_reason && order.payment_status === 'Rejected' && (
                    <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-lg mt-4">
                      <div className="text-xs uppercase tracking-widest text-red-500 mb-1">Reason for Rejection</div>
        ...
        <div className="space-y-8">
          <div className="surface-glass p-8 rounded-[24px] border border-[color:var(--border)] shadow-sm">
            ...
            <div className="mt-6 flex justify-between items-end">
              ...
              <span className="text-2xl tracking-widest text-[color:var(--foreground)] font-medium">₹{order.total}</span>
            </div>
          </div>
          ...
          <div className="surface-glass p-8 rounded-[24px] border border-[color:var(--border)] shadow-sm">
            ...
            {shipping ? (
              <div className="text-sm space-y-1 text-[color:var(--muted-foreground)] leading-relaxed">
                <div className="text-[color:var(--foreground)] font-medium tracking-wide mb-2">{shipping.full_name}</div>
                ...
                <div className="pt-2 mt-2 border-t border-[color:var(--border)] flex items-center gap-2">
                  <span className="text-xs uppercase tracking-widest opacity-60">Phone:</span> {shipping.phone}
                </div>
              </div>
            ) : (
              <div className="text-sm text-[color:var(--muted-foreground)]">No shipping information available.</div>
            )}
            
            {(order.courier_name || order.tracking_number) && (
              <div className="mt-6 pt-6 border-t border-[color:var(--border)] space-y-3">
                <h4 className="text-xs uppercase tracking-widest text-[color:var(--gold)]">Shipping Details</h4>
                {order.courier_name && (
