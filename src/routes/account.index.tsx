    ...
    return (
      <div className="surface-glass rounded-[24px] p-10 animate-pulse">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-32 bg-[color:var(--foreground)]/5 rounded-xl"></div>
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="text-eyebrow text-[color:var(--muted-foreground)] tracking-widest mb-1.5">
                Welcome back to your Atelier
              </div>
              <h1 className="text-display text-4xl text-[color:var(--foreground)]">
                {displayName}
              </h1>
          ...
          <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row gap-3 w-full md:w-auto">
            <Link
              ...
              className="btn-lux justify-center whitespace-nowrap shadow-md w-full md:w-auto py-4 md:py-3"
            >
              <ShoppingBag size={16} className="mr-2" /> Browse Collection
            </Link>
            <Link
              to="/customize"
              className="btn-ghost-lux justify-center whitespace-nowrap bg-black/5 dark:bg-white/5 w-full md:w-auto py-4 md:py-3"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          ...
          >
            <Link
              to={s.to}
              className="group block surface-glass rounded-[24px] p-6 border border-[color:var(--border)] hover:border-[color:var(--gold)]/30 transition-all duration-300 hover:shadow-md hover:-translate-y-1"
            >
              <s.icon
                size={20}
                className="text-[color:var(--muted-foreground)] group-hover:text-[color:var(--gold)] transition-colors mb-4"
              />
              ...
              <div className="text-xs tracking-widest uppercase text-[color:var(--muted-foreground)] group-hover:text-[color:var(--foreground)] transition-colors">
                {s.label}
              </div>
            </Link>
          </motion.div>
      ...
      >
        <div className="flex justify-between items-start mb-8">
          <div>
            <h2 className="text-display text-2xl text-[color:var(--foreground)]">
              Personal Information
            </h2>
            <div className="text-sm text-[color:var(--muted-foreground)] mt-2">
              Manage your atelier identity
            </div>
          </div>
          {!isEditing && (
            <button
              onClick={() => setIsEditing(true)}
              className="flex items-center gap-2 text-sm text-[color:var(--muted-foreground)] hover:text-[color:var(--gold)] transition-colors p-2 -mr-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5"
            >
              <Edit2 size={16} />{" "}
              <span className="hidden sm:inline font-medium tracking-wide">Edit Profile</span>
            </button>
          )}
        </div>
        ...
        {isEditing ? (
          ...
          >
            ...
            <div className="flex gap-4 pt-4">
              <button type="submit" disabled={updateProfile.isPending} className="btn-lux">
          ...
          <div className="grid sm:grid-cols-2 gap-y-8 gap-x-12">
            <div>
              ...
              <div className="text-lg text-[color:var(--foreground)]">
                {profile.full_name || "—"}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-sm text-[color:var(--muted-foreground)] font-mono bg-black/5 dark:bg-white/5 px-3 py-1 rounded inline-block">
