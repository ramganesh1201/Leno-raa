  ...
  const { data: analytics, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      verifiedOrders.forEach((o) => {
        ...
        const dayMatch = last30Days.find(
          (d) =>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <StatCard
          ...
          icon={DollarSign}
        />
        <StatCard title="Verified Orders" value={analytics?.orderCount} icon={ShoppingBag} />
        <StatCard
          title="Average Order Value"
          value={`₹${Math.round(analytics?.aov || 0).toLocaleString()}`}
          icon={Target}
        />
        <StatCard title="Total Customers" value={analytics?.customerCount} icon={Users} />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        />
        <StatCard title="Conversion Rate" value="N/A" icon={TrendingUp} subtitle="Coming soon" />
        <StatCard title="Avg. Verification Time" value="N/A" icon={Clock} subtitle="Coming soon" />
        <StatCard
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm p-6 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4">
            ...
            {analytics?.topProducts?.map((product, idx) => (
              <div
                key={idx}
                className="flex justify-between items-center bg-neutral-50 dark:bg-neutral-800/50 p-3 rounded-xl border border-neutral-200 dark:border-neutral-800"
              >
