...
import { Heart, ShoppingBag, Sparkles, X, HeartCrack, Search, SlidersHorizontal } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] bg-black/5 dark:bg-white/5 overflow-hidden">
        {(() => {
          const itemImage = product.image || product.image_url || product.cover_image || product.images?.[0] || product.ui_metadata?.image;
          return itemImage ? (
            <motion.img 
            ...
            <div className="w-full h-full flex items-center justify-center text-[color:var(--muted-foreground)]">No Image</div>
          );
        })()}
        ...
        <div className="absolute inset-x-4 bottom-4 flex gap-2 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
          ...
          >
            {isAdding ? "Adding..." : <><ShoppingBag size={14} /> Quick Add</>}
          </button>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col justify-between bg-gradient-to-b from-transparent to-black/5 dark:to-white/5">
        <div>
          <div className="flex justify-between items-start mb-2">
            <div>
              <div className="text-xs uppercase tracking-widest text-[color:var(--muted-foreground)] mb-1">{product.collection || 'Radiance'}</div>
              <Link to="/products/$slug" params={{ slug: product.slug }} className="text-lg font-medium text-[color:var(--foreground)] hover:text-[color:var(--gold)] transition-colors">
                {product.name}
              </Link>
            </div>
        ...
        <AnimatePresence mode="wait">
          {baseItems.length === 0 ? (
            ...
            >
              ...
              <h2 className="text-display text-2xl mb-3">You haven't discovered your ritual yet.</h2>
              ...
              <Link to="/collections/$slug" params={{ slug: "radiance" }} className="btn-lux inline-flex justify-center shadow-lg">
                Browse Collection
              </Link>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-black/5 dark:bg-white/5 p-4 rounded-2xl border border-[color:var(--border)]">
                <div className="flex items-center gap-2 text-sm text-[color:var(--muted-foreground)]">
                  ...
                  Last saved: <span className="font-medium text-[color:var(--foreground)]">{baseItems[0]?.name || "Recently"}</span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row w-full md:w-auto gap-4">
                  ...
                  <div className="relative">
                    <SlidersHorizontal size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--muted-foreground)]" />
                    <select
                      value={sortBy}
              ...
              {filteredAndSortedItems.length === 0 ? (
                <div className="text-center py-20">
                  <p className="text-[color:var(--muted-foreground)]">No rituals match your search.</p>
                </div>
              ) : (
                <motion.div 
                  layout
