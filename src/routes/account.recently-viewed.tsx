  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-end mb-6">
        ...
        <Link to="/collections/$slug" params={{ slug: "radiance" }} className="btn-ghost-lux hidden sm:inline-flex items-center gap-2">
          Continue Shopping <ArrowRight size={14} />
        </Link>
      </div>

      <div className="relative -mx-6 px-6 md:mx-0 md:px-0">
        <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pb-12 scrollbar-hide" style={{ scrollPaddingLeft: '1.5rem', scrollPaddingRight: '1.5rem' }}>
          {items.map((p, i) => (
            <motion.div 
              key={p.slug}
            ...
            >
              <Link to="/products/$slug" params={{ slug: p.slug }} className="block relative aspect-[4/5] rounded-[24px] overflow-hidden bg-black/5 dark:bg-white/5 shadow-sm border border-[color:var(--border)] group-hover:border-[color:var(--gold)]/30 transition-all duration-500">
                {p.image ? (
                  <motion.img 
                ...
                <div className="absolute top-4 left-4 bg-[color:var(--background)]/80 backdrop-blur-md px-3 py-1 rounded-full text-[10px] uppercase tracking-widest flex items-center gap-1.5 shadow-sm">
                  <Clock size={10} /> {i === 0 ? "Just now" : i === 1 ? "Earlier today" : "Previously"}
                </div>
              </Link>
              
              <div className="mt-6 flex flex-col items-center text-center">
                <div className="text-xs uppercase tracking-widest text-[color:var(--muted-foreground)] mb-2">{p.collection || 'Radiance'}</div>
                <Link to="/products/$slug" params={{ slug: p.slug }} className="text-lg font-medium text-[color:var(--foreground)] hover:text-[color:var(--gold)] transition-colors">
                  {p.name}
                </Link>
      ...
      <div className="sm:hidden flex justify-center pb-8">
        <Link to="/collections/$slug" params={{ slug: "radiance" }} className="btn-lux w-full justify-center shadow-lg">
          Continue Shopping
        </Link>
      </div>
    </div>
