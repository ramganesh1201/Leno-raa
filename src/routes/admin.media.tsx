...
import { UploadCloud, Search, Trash2, Link as LinkIcon, Image as ImageIcon, FileVideo, File } from "lucide-react";
...
function AdminMediaPage() {
  ...
  const [mediaFiles] = useState([
    { id: 1, name: "hero-banner-main.jpg", type: "image", size: "1.2 MB", date: "2024-05-10", url: "https://placehold.co/1920x1080/1a1a1a/ffffff?text=Hero" },
    { id: 2, name: "rose-clay-soap-1.jpg", type: "image", size: "450 KB", date: "2024-05-11", url: "https://placehold.co/800x800/f5e6e6/000000?text=Rose+Clay" },
    { id: 3, name: "lavender-dream-1.jpg", type: "image", size: "520 KB", date: "2024-05-11", url: "https://placehold.co/800x800/e6e6fa/000000?text=Lavender" },
    { id: 4, name: "brand-video-720p.mp4", type: "video", size: "15.4 MB", date: "2024-05-12", url: "#" },
    { id: 5, name: "upi-qr-code.png", type: "image", size: "120 KB", date: "2024-05-15", url: "https://placehold.co/400x400/ffffff/000000?text=QR" },
  ]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-neutral-500 mt-1">Manage all your product images, banners, and videos in one place.</p>
        </div>
      ...
      <AnimatePresence>
        {selectedFile && (
          <div className="fixed inset-0 z-[100] flex justify-end bg-black/50 backdrop-blur-sm" onClick={() => setSelectedFile(null)}>
            <motion.div 
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              <div className="p-4 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-center bg-neutral-50 dark:bg-neutral-950/50">
                ...
                <button onClick={() => setSelectedFile(null)} className="text-neutral-500 hover:text-neutral-900 dark:hover:text-white text-sm font-medium">Close</button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">File Name</label>
                    <div className="text-sm font-medium break-all">{selectedFile.name}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Type</label>
                      <div className="text-sm capitalize">{selectedFile.type}</div>
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Size</label>
                      <div className="text-sm">{selectedFile.size}</div>
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Uploaded</label>
                      <div className="text-sm">{new Date(selectedFile.date).toLocaleDateString()}</div>
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Public URL</label>
                    <div className="flex gap-2">
                      <input type="text" readOnly value={selectedFile.url} className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-2 text-xs font-mono outline-none" />
