...
import { CheckCircle2, XCircle, Search, Filter, ExternalLink, Download, Image as ImageIcon } from "lucide-react";
...
function AdminPaymentsPage() {
  ...
  const handleVerify = async (orderId: string, proofId: string, isApproved: boolean) => {
    let reason = "";
    if (!isApproved) {
      reason = prompt("Please provide a reason for rejection (e.g. Blurred image, UTR mismatch):") || "Invalid payment proof";
    }
  ...
  const handleRequestNew = async (orderId: string, proofId: string) => {
    const reason = prompt("Reason for requesting new screenshot:") || "Please upload a clearer screenshot.";
    await verifyPayment.mutateAsync({ orderId, proofId, isApproved: false, rejectionReason: reason });
    setSelectedProof(null);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-neutral-500 mt-1">Review customer payment screenshots and approve orders.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredPayments.length === 0 ? (
          <div className="col-span-full py-20 text-center bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
            <CheckCircle2 size={48} className="mx-auto text-neutral-300 dark:text-neutral-700 mb-4" />
          ...
          filteredPayments.map((payment: any) => (
            <div key={payment.id} className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm overflow-hidden flex flex-col hover:border-blue-500/50 transition-colors cursor-pointer" onClick={() => setSelectedProof(payment)}>
              <div className="h-48 bg-neutral-100 dark:bg-neutral-800 relative group overflow-hidden">
                ...
                <div className="absolute top-2 right-2">
                  <span className={`inline-flex px-2 py-1 rounded-full text-[10px] uppercase tracking-widest font-medium border backdrop-blur-md ${
                    payment.verification_status === 'verified' ? 'bg-emerald-500/20 text-emerald-100 border-emerald-500/30' : 
                    payment.verification_status === 'rejected' ? 'bg-rose-500/20 text-rose-100 border-rose-500/30' :
              ...
              <div className="p-4 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="font-bold text-blue-600 dark:text-blue-400">₹{payment.order?.total}</div>
                </div>
                <div className="text-xs text-neutral-500 mb-1">UTR: <span className="text-neutral-900 dark:text-neutral-200 font-medium">{payment.utr_number}</span></div>
                <div className="text-xs text-neutral-500 mb-4">{new Date(payment.uploaded_at).toLocaleString()}</div>
                
                {payment.verification_status === 'pending' && (
                  <div className="mt-auto grid grid-cols-2 gap-2">
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleVerify(payment.order_id, payment.id, true); }}
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleVerify(payment.order_id, payment.id, false); }}
      ...
      <AnimatePresence>
        {selectedProof && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedProof(null)}>
            <motion.div 
            ...
            >
              ...
              <div className="w-full md:w-96 flex flex-col border-l border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
                <div className="flex items-center justify-between p-4 border-b border-neutral-200 dark:border-neutral-800">
                  ...
                  <button onClick={() => setSelectedProof(null)} className="text-neutral-500 hover:text-neutral-900 dark:hover:text-white">
                    <XCircle size={24} />
                  </button>
                </div>
                ...
                <div className="p-6 flex-1 overflow-y-auto space-y-6">
                  <div>
                    <div className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Status</div>
                    <span className={`inline-flex px-3 py-1 rounded-full text-xs uppercase tracking-widest font-medium border ${
                      selectedProof.verification_status === 'verified' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 
                      selectedProof.verification_status === 'rejected' ? 'bg-rose-500/10 text-rose-600 border-rose-500/20' :
                  ...
                  <div>
                    <div className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-1">Order Total</div>
                    <div className="text-3xl font-bold text-neutral-900 dark:text-white">₹{selectedProof.order?.total}</div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 space-y-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm">{new Date(selectedProof.uploaded_at).toLocaleString()}</div>
                      </div>
                    </div>

                    <div>
                      <div className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-2">Customer</div>
                      <div className="text-sm">
                        <div>{selectedProof.order?.shipping_addresses?.[0]?.full_name}</div>
                        <div className="text-neutral-500">{selectedProof.order?.profiles?.email}</div>
                        <div className="text-neutral-500">{selectedProof.order?.shipping_addresses?.[0]?.phone}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {selectedProof.verification_status === 'pending' && (
                  <div className="p-4 border-t border-neutral-200 dark:border-neutral-800 space-y-3">
                    ...
                    <div className="flex gap-3">
                      <button 
                        onClick={() => handleVerify(selectedProof.order_id, selectedProof.id, false)}
