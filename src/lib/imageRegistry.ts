import allSoaps from "@/assets/all-soaps.webp";
import envCalm from "@/assets/env-calm.webp";
import envHerbal from "@/assets/env-herbal.webp";
import envNourish from "@/assets/env-nourish.webp";
import envRadiance from "@/assets/env-radiance.webp";
import envRelax from "@/assets/env-relax.webp";
import heroIntro from "@/assets/hero-intro.webp";

import aloeVeraGel from "@/assets/soaps/aloeveragel.webp";
import aloeVera from "@/assets/soaps/aloeVera.webp";
import ayuHerbal from "@/assets/soaps/ayuherbal.webp";
import butterSoap from "@/assets/soaps/buttersoap.webp";
import charcoal from "@/assets/soaps/charcoal.webp";
import coffee from "@/assets/soaps/coffee.webp";
import goatMilk from "@/assets/soaps/goatmilk.webp";
import lavender from "@/assets/soaps/lavender.webp";
import manji from "@/assets/soaps/manji.webp";
import manjista from "@/assets/soaps/manjista.webp";
import menthol from "@/assets/soaps/menthol.webp";
import nalpamaradhi from "@/assets/soaps/nalpamaradhi.webp";
import oats from "@/assets/soaps/oats.webp";
import orange from "@/assets/soaps/orange.webp";
import rose from "@/assets/soaps/rose.webp";
import tomato from "@/assets/soaps/tomato.webp";

export const IMAGE_REGISTRY: Record<string, string> = {
  "all-soaps.webp": allSoaps,
  "env-calm.webp": envCalm,
  "env-herbal.webp": envHerbal,
  "env-nourish.webp": envNourish,
  "env-radiance.webp": envRadiance,
  "env-relax.webp": envRelax,
  "hero-intro.webp": heroIntro,

  "aloeveragel.webp": aloeVeraGel,
  "aloeVera.webp": aloeVera,
  "ayuherbal.webp": ayuHerbal,
  "buttersoap.webp": butterSoap,
  "charcoal.webp": charcoal,
  "coffee.webp": coffee,
  "goatmilk.webp": goatMilk,
  "lavender.webp": lavender,
  "manji.webp": manji,
  "manjista.webp": manjista,
  "menthol.webp": menthol,
  "nalpamaradhi.webp": nalpamaradhi,
  "oats.webp": oats,
  "orange.webp": orange,
  "rose.webp": rose,
  "tomato.webp": tomato,
};
