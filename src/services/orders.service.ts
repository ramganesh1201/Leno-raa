...
export const ordersService = {
  ...
  async createOrder(shippingDetails: { full_name: string; phone: string; address: string; city: string; state: string; pincode: string }, subtotal: number, shipping_cost: number, tax: number = 0, discount: number = 0) {
    const { data: { user } } = await supabase.auth.getUser();
  ...
  async getAdminOrders() {
    ...
    const { data: profiles } = await supabase.from("profiles").select('id, email, full_name, phone');
    const { data: orderItems } = await supabase.from("order_items").select('*, product:products(*), customization:soap_customizations(*)');
  ...
  async getAdminPayments() {
    ...
    const { data: orders } = await supabase.from("orders").select('id, order_number, total, payment_status, user_id');
    const { data: profiles } = await supabase.from("profiles").select('id, email, full_name, phone');
    const { data: shipping } = await supabase.from("shipping_addresses").select('order_id, full_name, phone');
  ...
  async verifyPayment(orderId: string, proofId: string, isApproved: boolean, rejectionReason?: string) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Not authenticated");
  ...
  async updateOrderStatus(orderId: string, status: string, notes?: string, courier?: string, tracking?: string) {
